...
for SNAPSHOT in ${SNAPSHOT_LIST}; do
    ...
    if [[ "$2" == "--dry-run" ]]; then
        ...
        envsubst < gce_batch_job.json | gcloud --billing-project protocol-labs-data beta batch jobs submit lily-backfill-snapshot-"$PADDED_SNAPSHOT_EPOCH_FROM"-"$PADDED_SNAPSHOT_EPOCH_TO"-"$(date +%s)" --location us-central1 --config=-
    fi
done
